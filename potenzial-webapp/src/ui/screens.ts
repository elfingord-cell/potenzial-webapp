import { formatCurrency } from "../models/entry";
...
function renderProgressRing(progressPercent: number, progressRingPercent: number): string {
  return `
    <div class="relative w-full max-w-[320px] aspect-square flex items-center justify-center mb-6">
      <svg class="w-full h-full transform -rotate-90" viewBox="0 0 36 36" aria-label="${progressPercent}% complete">
        <path
...
export function renderHomeScreen(data: HomeScreenData): string {
  const recentContent = data.recentEntries.length
    ...
    : renderEmptyCard("No activity yet. Tap + to add your first savings entry.");
...
export function renderEntriesScreen(data: EntriesScreenData): string {
  const changeText = formatChangeText(data.monthChangePercent, "from last month");
  const entriesContent = data.entries.length
    ? data.entries.map((entry) => renderEntryRow(entry, true)).join("")
    : renderEmptyCard("No entries yet. Add entries to build your activity history.");
    ...
    <main class="flex-1 overflow-y-auto pb-36">
      <div class="flex flex-wrap gap-4 p-4">
        <section class="flex min-w-[158px] flex-1 flex-col gap-1 rounded-xl p-5 bg-white border border-slate-100 shadow-sm">
          ...
          <p class="text-primary text-sm font-medium bg-primary/10 w-fit px-2 py-0.5 rounded text-[12px]">${escapeHtml(formatChangeText(insights.weekChangePercent, "vs last week"))}</p>
        </section>
        ...
        <section class="flex min-w-[158px] flex-1 flex-col gap-1 rounded-xl p-5 bg-white border border-slate-100 shadow-sm">
          ...
          <p class="text-primary text-sm font-medium bg-primary/10 w-fit px-2 py-0.5 rounded text-[12px]">${escapeHtml(formatChangeText(insights.monthChangePercent, "vs last month"))}</p>
        </section>
      </div>
        ...
        <div class="bg-white rounded-xl p-6 border border-slate-100 shadow-sm">
          <div class="flex items-end justify-between mb-6">
            <div class="flex flex-col gap-1">
              ...
              <p class="text-slate-900 tracking-tight text-5xl font-bold truncate">${formatCurrency(insights.last7Total)}</p>
            </div>
      ...
      <section class="p-4 flex flex-col gap-3">
        ${renderStatRow("savings", "Average Daily Saved", formatMoney(insights.averageDailySaved))}
        ${renderStatRow("local_offer", "Top Category", insights.topCategory)}
        ${renderStatRow("flag", "Projected Monthly", formatMoney(insights.projectedMonthly))}
      </section>
    </main>
  `;
}
...
export function renderEntrySheet(sheet: EntrySheetViewModel): string {
  ...
  const amountLabel = sheet.type === "income" ? "Income amount" : "Amount saved";
  const title = sheet.mode === "edit" ? "Edit Savings" : "Add Savings";
  const buttonText = sheet.mode === "edit" ? "Save Changes" : "Save Entry";
